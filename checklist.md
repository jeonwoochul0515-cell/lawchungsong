...
## 언론·단체 게재글 백링크 요청 (2026-07-17) 진행 중
- [x] 사용자 질문("링크 없이 이름만 언급돼도 점수 오르나") 답변: 백링크 점수엔 거의 안 오르지만 AEO·GEO(엔티티 신뢰도) 점수엔 이미 기여 중이라고 설명
- [x] `외부인용_링크목록.md` 중 실제 요청 가능성 높은 곳 선별: 대형 언론(법률신문·국제신문·중앙일보)은 정정 잘 안 해줘서 제외, 소규모 매체·단체 위주로 선정
- [x] 더버터(hello@thebutter.org), 이젠센터(egenhappy@egen.or.kr) 이메일 확인, 요청 문구 작성해 사용자에게 전달(Gmail 권한 문제로 직접 발송은 못함)
- [ ] 부산시 공익제보센터(청렴담당관) — 이메일 못 찾음, 전화(051-888-1674/1718 혼재)만 확인됨. 민원포털은 본인인증 필요할 가능성 높아 후순위
- **대기**: 사용자가 더버터·이젠센터에 메일 발송 여부

---

## 미해결 결정 / 입력 대기
- [ ] 더버터·이젠센터 메일 발송 및 반영 여부 확인
